...
/* Le thread a besoin de connaître son image ET le compteur partagé */
typedef struct {
    int numero_image;
    int *compteur_partage;   /* pointeur vers la mémoire partagée */
} TacheThread;
...
void* tache_thread(void *arg) {
    ...
    if (img) {
        ...
        /* Incrémenter le compteur global en mémoire partagée */
...
int main(int argc, char *argv[]) {
    ...
    /* ===== Créer la mémoire partagée AVANT le fork ===== */
    key_t cle = ftok(".", 'A');
    ...
    *compteur = 0;   /* compteur global à 0 */
    ...
    for (int p = 0; p < nb_processus; p++) {
        ...
        if (pid == 0) {
            ...
            for (int t = 0; t < nb_threads; t++) {
                ...
                taches[t].compteur_partage = compteur;  /* donner accès au compteur */
            ...
            shmdt(compteur);   /* détacher dans l'enfant */
            exit(0);
        }
    }
    ...
    /* Le parent lit le total directement dans la mémoire partagée */
    printf("Parent : compteur global = %d images traitees\n", *compteur);

    shmdt(compteur);                  /* détacher */
    shmctl(shmid, IPC_RMID, NULL);    /* supprimer la zone partagée */
